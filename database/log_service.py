...
def log_email_send(customer_id, email_id, task_id, source, subject, content,
                   status, error_message=None, conn=None, user_id=None, recipient_email=None):
    """
    统一写入 email_logs 表
    ...
    try:
        ...
        cursor.execute('''
            ...
            (customer_id, email_id, task_id, source, email_subject, email_content,
             send_status, error_message, sent_at, created_at, user_id, recipient_email)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, ?, ?)
        ''', (customer_id, email_id, task_id, source, subject, content,
              status, error_message,
              datetime.now().isoformat() if status == 'sent' else None, user_id, recipient_email))
        conn.commit()
