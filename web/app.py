...
def api_stats():
    ...
    try:
        ...
        # 主题池统计
        cursor.execute("SELECT COUNT(DISTINCT customer_id) FROM customer_subjects")
    ...
    try:
        ...
        cursor.execute("""
            INSERT INTO customers (customer_name, country, address, website, company_info,
                supplier, supplier_info, customs_data, logistics_info, industry_type, created_at, updated_at)
        ...
        for email_data in emails:
            ...
            if contact_name:
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, datetime('now'))
                """, (customer_id, contact_name, job_title, email_data.get('source', 'manual')))
                contact_id = cursor.lastrowid

            # 插入邮箱（同时保存contact_name和job_title到emails表）
            cursor.execute("""
                INSERT INTO emails (customer_id, contact_id, email_address, email_type,
                                    contact_name, job_title, is_active, created_at)
                VALUES (?, ?, ?, ?, ?, ?, 1, datetime('now'))
            """, (customer_id, contact_id, email_addr, email_type, contact_name, job_title))
        ...
        try:
            ...
            for customer_data in customers:
                try:
                    ...
                    # 检查重复
                    is_dup, dup_id = check_duplicate_customer_name(customer_data['customer_name'])
                    if is_dup:
                        fail_count += 1
                        fail_details.append({
                    ...
                    cursor.execute("""
                        INSERT INTO customers (customer_name, country, address, website, company_info,
                            supplier, supplier_info, customs_data, logistics_info, created_at, updated_at)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), datetime('now'))
                    """, (
...
def api_check_duplicate():
    ...
    try:
        ...
        if name:
            is_dup, dup_id = check_duplicate_customer_name(name)
            result['name_duplicate'] = is_dup
            result['name_duplicate_id'] = dup_id

        if email:
            is_dup, dup_info = check_duplicate_email(email)
...
def api_get_blacklist():
    ...
    page = request.args.get('page', 1, type=int)
    per_page = request.args.get('per_page', 20, type=int)
    data = get_blacklist(page=page, per_page=per_page)
...
def api_add_blacklist():
    ...
    if add_blacklist(company_name, website, reason):
...
def api_remove_blacklist():
    ...
    if not company_name:
        return jsonify({'success': False, 'error': '公司名称不能为空'}), 400
    if remove_blacklist(company_name, website):
...
def api_check_blacklist():
    ...
    return jsonify({'success': True, 'blacklisted': is_blacklisted(name, website)})
...
def api_bounce_list():
    ...
    data = get_bounce_list(page, per_page, bounce_type)
...
def api_create_config():
    ...
    if create_api_config(api_name, api_key, base_url, model):
...
def api_customer_email_add(customer_id):
    ...
    try:
        ...
        # 检查该客户下是否已存在相同邮箱
        is_dup, dup_info = check_duplicate_email(email_addr, customer_id)
        if is_dup:
            conn.close()
        ...
        if contact_name:
            cursor.execute("""
                INSERT INTO contacts (customer_id, contact_name, job_title, source, created_at)
                VALUES (?, ?, ?, ?, datetime('now'))
            """, (customer_id, contact_name, job_title, 'manual'))
            contact_id = cursor.lastrowid
...
def api_subjects():
    ...
    try:
        ...
        # 主题类型分布
        cursor.execute('SELECT subject_type, COUNT(*) FROM customer_subjects GROUP BY subject_type')
...
def api_countries():
    ...
    try:
        ...
        cursor.execute('''
            ...
            FROM customers c
            WHERE c.country IS NOT NULL AND c.country != '' AND c.country != 'nan' AND c.country != 'NaN' AND c.country != 'None'
            GROUP BY c.country
            ORDER BY cnt DESC
        ''')
        rows = cursor.fetchall()
    ...
    try:
        ...
        cursor.execute('''
            ...
            WHERE c.country IS NOT NULL AND c.country != ''
                  AND c.country != 'nan' AND c.country != 'NaN' AND c.country != 'None'
            GROUP BY c.country
...
def api_create_send_task():
    ...
    try:
        ...
        thread = threading.Thread(
            ...
            args=(task_id, emails_to_send, send_config, target_word_count),
            daemon=True
        )
        thread.start()
...
def _do_batch_send(task_id, emails_to_send, send_config, target_word_count=None):
    """后台执行批量发送任务 - 完整邮件工作流"""
    ...
    try:
        ...
        if not all_email_items:
            ...
            persist_send_task_meta(task_id, 'batch', 'failed', error='所有客户邮件生成失败')
            return
        ...
        import traceback
        traceback.print_exc()
        persist_send_task_meta(task_id, 'batch', 'failed', error=str(e))
...
def api_get_send_task(task_id):
    ...
    try:
        ...
        # 从数据库获取
        db_tasks = get_active_send_tasks()
...
def api_send_task_action(task_id):
    ...
    try:
        ...
        if success:
            persist_send_task_meta(task_id, 'batch', action + 'ed' if action != 'cancel' else 'cancelled')
        return jsonify({'success': success, 'message': message})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)}), 500
    ...
    try:
        ...
        for cr in customer_rows:
            ...
            cursor.execute('''
                ...
                FROM email_logs el
                LEFT JOIN emails e ON el.email_id = e.id
                WHERE el.customer_id = ? AND el.source = 'scheduled'
                ORDER BY el.sent_at DESC
                LIMIT 50
            ''', (customer_id,))
            email_rows = cursor.fetchall()
...
def api_email_log_scheduled_detail(log_id):
    ...
    try:
        ...
        cursor.execute('''
            ...
            LEFT JOIN customers c ON el.customer_id = c.id
            LEFT JOIN emails e ON el.email_id = e.id
            WHERE el.id = ? AND el.source = 'scheduled'
        ''', (log_id,))
        row = cursor.fetchone()
        conn.close()
...
def api_diagnostics_scheduled_logs():
    ...
    try:
        ...
        # 总记录数
        cursor.execute('SELECT COUNT(*) FROM email_logs WHERE source = ?', ('scheduled',))
        total = cursor.fetchone()[0]
...
def api_cooldown_status():
    ...
    try:
        ...
        # 统计总客户数（有活跃邮箱的客户）
        cursor.execute('''
            SELECT COUNT(DISTINCT customer_id) FROM emails WHERE is_active = 1
        ''')
        total_customers = cursor.fetchone()[0]
        ...
        cursor.execute('''
            ...
            LEFT JOIN email_logs el ON c.id = el.customer_id AND el.send_status = 'sent'
            WHERE c.id NOT IN (SELECT customer_id FROM cooldown_override)
            GROUP BY c.id
        ''')
        rows = cursor.fetchall()
...
def api_send_test():
    ...
    try:
        ...
        thread = threading.Thread(
            ...
            args=(task_id, customer_id, email_addresses),
            daemon=True
        )
        thread.start()
...
def _do_send_email(task_id, customer_id, email_addresses=None):
    """后台执行邮件发送"""
    task = send_tasks.get(task_id)
    if not task:
        return
    ...
    try:
        ...
        email_content = workflow.generate_email(customer_name, website or '', progress_callback=on_progress, target_word_count=task.get('target_word_count'))
        ...
        import traceback
        traceback.print_exc()
        persist_send_task_meta(task_id, 'manual', 'failed', error=str(e))
...
def api_send_task_detail(task_id):
    ...
    try:
        ...
        if not task:
            return jsonify({'success': False, 'error': '任务不存在'}), 404
        items = get_send_task_items(task_id)
        return jsonify({'success': True, 'data': {
...
def api_send_pause(task_id):
    ...
    try:
        success = queue_manager.pause_task(task_id)
        if success:
            persist_send_task_meta(task_id, 'manual', 'paused')
...
def api_send_cancel(task_id):
    ...
    try:
        success = queue_manager.cancel_task(task_id)
        if success:
            persist_send_task_meta(task_id, 'manual', 'cancelled')
...
def _do_generate_preview(task_id, customer_id):
    """后台生成邮件预览内容"""
    task = send_tasks.get(task_id)
    if not task:
        return
...
def api_email_send():
    ...
    try:
        ...
        thread = threading.Thread(
            ...
            args=(task_id, customer_id, email_addresses, subject, body),
            daemon=True
        )
        thread.start()
...
def _do_send_custom_email(task_id, customer_id, email_addresses, subject, body):
...
def api_material_update(material_id):
    ...
    try:
        data = request.json or {}
        update_material(material_id, data)
...
def api_materials_analyze():
    ...
    try:
        ...
        result = analyzer.analyze_file(file_bytes, file.filename)
...
def _do_batch_import(task_id, temp_path, filename):
    """后台执行批量导入任务"""
            ...
            try:
                ...
                if existing:
                    ...
                    cursor.execute('''
                        ...
                        (customer_name, country, website, address, company_info, industry_type)
                        VALUES (?, ?, ?, ?, ?, ?)
                    ''', (
                        customer['customer_name'],
                ...
                for contact in contacts:
                    ...
                    try:
                        ...
                        if not existing_email:
                            cursor.execute('''
                                INSERT INTO emails
                                (customer_id, email_address, email_type, contact_name, job_title, is_active)
                                VALUES (?, ?, ?, ?, ?, 1)
                            ''', (
                                customer_id,
                                email,
...
def api_search_result_detail(result_id):
    ...
    result = get_search_result(result_id)
...
def api_search_results_bulk_review():
    ...
    updated = bulk_update_result_status(result_ids, status)
...
def api_search_results_import():
    ...
    for rid in result_ids:
        try:
            result = import_result_to_customer(rid, options)
            if result['success']:
                imported += 1
